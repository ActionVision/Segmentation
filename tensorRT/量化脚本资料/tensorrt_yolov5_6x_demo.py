...
def gpu_trt_demo():
    ...
    with open("uav_bird_int8.engine", 'rb') as f, trt.Runtime(logger) as runtime:
    ...
    capture = cv.VideoCapture("D:/bird_test/bird_fly.mp4")
    ...
    while True:
        ...
        cv.imshow("YOLOv5s 7.0 + TensorRT8.6.x by gloomyfish", frame)
        cc = cv.waitKey(1)
        if cc == 27:
            break
    cv.waitKey(0)
