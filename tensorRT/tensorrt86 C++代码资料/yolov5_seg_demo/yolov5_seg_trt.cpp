...
YOLOv5TRTSegment::~YOLOv5TRTSegment() {
	// 同步结束，释放资源
...
void YOLOv5TRTSegment::initConfig(std::string enginefile, float conf_thresholod, float score_thresholod) {
...
float YOLOv5TRTSegment::sigmoid_function(float a)
{
	float b = 1. / (1. + exp(-a));
	return b;
}

void YOLOv5TRTSegment::detect(cv::Mat &frame, std::vector<DetectResult> &results) {
	cv::RNG rng;
	...
	for (int i = 0; i < det_output.rows; i++) {
		...
		cv::Mat classes_scores = det_output.row(i).colRange(5, output_w-32);
		...
		if (score > this->score_thresholod)
		{
			...
			box.width = width;
			box.height = height;
			cv::Mat mask2 = det_output.row(i).colRange(output_w - 32, output_w);
			boxes.push_back(box);
